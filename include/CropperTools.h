...
void http_output_image(const Imagine::NFmiImage & theImage,
					   ...
					   bool theCacheFlag);
void parse_geometry(const std::string & theGeometry,
					int & x1,
					int & y1,
					int & width,
					int & height);
void parse_center_geometry(const std::string & theGeometry,
						   int & xc,
						   int & yc,
						   int & width,
						   int & height);
...
std::auto_ptr<Imagine::NFmiImage> crop_corner(const Imagine::NFmiImage & theImage,
											  ...
											  int & theXoff,
											  int & theYoff);
std::auto_ptr<Imagine::NFmiImage> crop_center(const Imagine::NFmiImage & theImage,
											  int theXC,
											  int theYC,
											  int theWidth,
											  int theHeight,
